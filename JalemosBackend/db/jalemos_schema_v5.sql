-- ============================================================
--  JALEMOS — Full schema v5 (create from scratch)
--
--  Changes vs v4:
--    · users: qr_token UUID UNIQUE (stable per-user QR identifier)
--    · trip_state enum: adds 'boarding' phase
--    · trips: boarding_started_at, journey_started_at, completed_at,
--             cancelled_at, cancel_reason, cancel_details
--    · booking_state enum: adds 'boarded', 'no_show'
--    · bookings: boarded_at, cancel_reason, cancel_details
--    · notification_type enum: 8 new lifecycle event types
--    · notifications: body TEXT column
--    · fn_update_available_seats: does not free seats on boarded→cancelled
-- ============================================================
...
-- 3. TABLE: trips
-- ============================================================
CREATE TYPE trip_state AS ENUM ('scheduled', 'boarding', 'in_progress', 'completed', 'cancelled');
...
-- 4. TABLE: bookings
-- ============================================================
CREATE TYPE booking_state AS ENUM ('pending', 'confirmed', 'boarded', 'no_show', 'cancelled', 'completed');
...
--   Only pending/confirmed → cancelled releases seats.
--   boarded → cancelled does NOT free seats (passenger was in vehicle).
-- ============================================================
...
BEGIN
    ...
    -- Solicitud de vehículo adicional (pendiente)
    INSERT INTO driver_applications (
        user_id, status, attempts, application_type,
        vehicle_brand, vehicle_model, vehicle_year, vehicle_plate, vehicle_color,
    ...
    -- NOTIFICACIONES
    INSERT INTO notifications (user_id, trip_id, type, title, body) VALUES
        (v_alvaro_id, v_trip1, 'booking_confirmed',
         'Tu reserva con Carlos Monestel fue confirmada',
         'Tienes un lugar reservado para el viaje UCR → Cartago'),
        (v_carlos_id, v_trip1, 'booking_received',
         'Álvaro Moya reservó un lugar en tu viaje',
         'Revisa tu pantalla de abordaje cuando llegue la hora del viaje');
