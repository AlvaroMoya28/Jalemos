-- ============================================================
--  JALEMOS — Full schema v4
--  Crea la base de datos desde cero con datos completos.
--
--  Cambios respecto a v3:
--    · driver_applications: nueva columna application_type
--      ('driver' = solicitud inicial de conductor,
--       'vehicle' = solicitud de vehículo adicional por conductor ya aprobado)
--    · driver_applications: cedula y address ahora tienen DEFAULT ''
--      para que las solicitudes tipo 'vehicle' no requieran esos campos.
--    · Seed actualizado con application_type explícito en todas las filas.
-- ============================================================
...
--
--  application_type:
--    'driver'  → solicitud inicial para convertirse en conductor
--    'vehicle' → solicitud de vehículo adicional (conductor ya aprobado)
--
--  Para solicitudes tipo 'vehicle', cedula y address no aplican
--  y se almacenan como cadena vacía (valor por defecto '').
-- ============================================================
...
CREATE TABLE driver_applications (
    ...
    -- Solo relevantes para application_type = 'driver'
...
COMMENT ON TABLE  driver_applications                    IS 'Solicitudes de conductor y de vehículo adicional';
COMMENT ON COLUMN driver_applications.application_type   IS '''driver'' = registro inicial; ''vehicle'' = vehículo adicional de conductor ya aprobado';
COMMENT ON COLUMN driver_applications.cedula             IS 'Número de cédula — solo para application_type=driver';
...
BEGIN
    ...
    -- José: Dekra VENCIDA (prueba del banner de advertencia en perfil)
    INSERT INTO users (username, email, password_hash, first_name, last_name, role,
                       mean_rating, total_trips, driver_trips, kms,
                       license_expiry_month, license_expiry_year,
                       dekra_expiry_month,   dekra_expiry_year)
    ...
    -- Carlos: Corolla (principal) + Accent (adicional — demuestra multi-vehículo)
    ...
    -- SOLICITUDES DE CONDUCTOR (aprobadas, application_type = 'driver')
    ...
    -- SOLICITUD DE VEHÍCULO ADICIONAL (ejemplo: Carlos pide agregar un tercer vehículo)
    INSERT INTO driver_applications (
        user_id, status, attempts, application_type,
        vehicle_brand, vehicle_model, vehicle_year, vehicle_plate, vehicle_color,
...
SELECT
    ...
    (SELECT COUNT(*) FROM driver_applications da WHERE da.user_id = u.user_id
                                                   AND da.application_type = 'driver') AS solicitudes_conductor,
    (SELECT COUNT(*) FROM driver_applications da WHERE da.user_id = u.user_id
                                                   AND da.application_type = 'vehicle') AS solicitudes_vehiculo,
