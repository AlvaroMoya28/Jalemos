...
export default function InteractiveMapModal({ visible, onClose, ride, polyline }: MapModalProps) {
  const insets = useSafeAreaInsets();
  const { isDark } = useAppTheme();
  const mapRef = useRef<MapView>(null);
  const routeCoords = useMemo(() => (polyline ? decodePolyline(polyline) : []), [polyline]);

  const origin = { latitude: ride.fromCoords.lat, longitude: ride.fromCoords.lng };
  const destination = { latitude: ride.toCoords.lat, longitude: ride.toCoords.lng };
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <View style={{ flex: 1 }}>
        <MapView
          ...
          showsScale
        >
          <Marker coordinate={origin} title={ride.from} pinColor="#1a9e8f" />
          <Marker coordinate={destination} title={ride.to} pinColor="#E53935" />
